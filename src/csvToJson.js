...
const joinMergeFieldMain = (srcObject, configObj) => {
  ...
  if (typeof configObj != "boolean" && typeof configObj == "object") {
    if(Array.isArray(configObj)){
      if(configObj.length>0){
        configObj.forEach((lpobj)=>{
          jsonObject = joinMergeField(jsonObject, lpobj);
        })
      }
    }else{
      jsonObject = joinMergeField(srcObject, configObj);
    }
  }
...
const convertOthersMain = (jsonObject, configObj) => {
  //console.log(jsonObject, configObj);
  if (!jsonObject || !configObj) {
    return jsonObject;
  }
  ...
  if (typeof configObj != "boolean" && typeof configObj == "object") {
    ...
    cleanKeys.forEach((propertyName) => {
      ...
      if (targetformat) {
        ...
        if(targetformat=="md5"){
          convothers = crypto.createHash('md5').update(jsonObject[propertyName]).digest("hex");
        }
    ...
    for (let i = index + 1; i < lines.length; i++) {
      ...
      if (stringUtils.hasContent(currentLine)) {
        ...
        smtData = this.addCustomFieldActive
          ...
          : smtData;
        let sssjsonObject = joinMergeFieldMain(smtData, this.joinFieldActive);
